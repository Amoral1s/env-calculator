jQuery(document).ready(function ($) {
  ...
  const inputCount = document.querySelector('input[name="Coupon_count"]');
  const inputPrice = document.querySelector('input[name="Coupon_price"]'); */
  ...
  $('.square-submit').on('click', function() {
    stripeInputs.forEach(elem => {
      if (elem.checked && elem.id == 'coupon-1'){
        $('.popup.sq-coupon-form.first').fadeIn(200);
        $('.overlay').fadeIn(200);
        calculate(elem.value, elem.dataset.old, quantityInput.value, elem.dataset.name);
      } else if (elem.checked && elem.id == 'coupon-2') {
        $('.popup.sq-coupon-form.second').fadeIn(200);
        $('.overlay').fadeIn(200);
        calculate(elem.value, elem.dataset.old, quantityInput.value, elem.dataset.name);
      }
    });
