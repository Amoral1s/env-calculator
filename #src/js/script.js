jQuery(document).ready(function ($) {
	//Цвета диаграмм 
	const E_block_colors = ['#586E2E', '#769241', '#93B94A', '#ABC971', '#BBDA7E'];
	const S_block_colors = ['#7D2F00', '#9F430C', '#C45C1D', '#DD7230', '#DB9065', '#F4BF9F'];
	const G_block_colors = ['#383C45', '#485053', '#5A626D', '#666A75', '#6F7F8E', '#828991', '#9DA5A7', '#9D9EA0'];
	...
	let params = {
		...
		'Потребительский сектор': ['E1.1', 'E1.2', 'E1.3', 'E1.4', 'E1.5', 'E1.6', 'E1.8',
			...
			'E3.1', 'E3.2', 'E3.3', 'E3.4', 'E3.5', 'E3.6', 'E3.7', 'E3.9', 'E3.10',
			'E4.1', 'E4.2', 'E4.3', 'E4.5', 'E4.6', 'E4.7', 'E4.8', 'E4.9', 'E4.10', 'E4.12', 'E4.13',
			'E5.1',
			'S1.1', 'S1.2', 'S1.3', 'S1.4',
			'S2.1', 'S2.2', 'S2.3', 'S2.4', 'S2.5', 'S2.6', 'S2.7', 'S2.8', 'S2.9', 'S2.10', 'S2.11', 'S2.12', 'S2.13', 'S2.14', 'S2.15', 'S2.16', 'S2.17',
			'S3.1', 'S3.2', 'S3.3', 'S3.4', 'S3.5', 'S3.6',
		...
		'Нефинансовые услуги': ['E1.1', 'E1.2', 'E1.3', 'E1.4', 'E1.5', 'E1.6', 'E1.8',
			...
			'E5.1',
			'S1.1', 'S1.2', 'S1.3', 'S1.4',
			'S2.1', 'S2.2', 'S2.3', 'S2.4', 'S2.5', 'S2.6', 'S2.7', 'S2.8', 'S2.9', 'S2.10', 'S2.11', 'S2.13', 'S2.14', 'S2.15', 'S2.16', 'S2.17',
			'S3.1', 'S3.2', 'S3.3', 'S3.4', 'S3.5', 'S3.6',
	...
	const calcStart = () => {
		...
		if (finance === false) { //определение категории компании
	...
	const miniNextFunc = () => {
		document.addEventListener('change', (event) => {
			...
			let targetWrapNav = target.closest('.wrap-nav');
			if (target.closest('.active-question')) {
				target.closest('.active-question').classList.add('checked-question');
			} else {
				return
			}
			const activeQuestionsLength = targetWrapNav.querySelectorAll('.active-question');
			const activeQuestionsChecked = targetWrapNav.querySelectorAll('.checked-question');
			let step = target.closest('.step');


			if (activeQuestionsChecked.length == activeQuestionsLength.length) {
				const miniNextBtn = targetWrapNav.querySelector('.mini-next');
				miniNextBtn.classList.remove('disabled');
				miniNextBtn.addEventListener('click', () => {
					if (targetWrapNav.nextElementSibling.classList.contains('button')) {
						miniNextBtn.style.display = 'none';
						targetWrapNav.nextElementSibling.click();
					} else {
						targetWrapNav.nextElementSibling.style.display = 'block';
						targetWrapNav.style.display = 'none';
						if (targetWrapNav.previousElementSibling.classList.contains('calc-wrapper-item') && !targetWrapNav.previousElementSibling.classList.contains('wrap-nav')) {
							targetWrapNav.previousElementSibling.style.display = 'none';
						}
	...
	const calcRenderItems = () => {
		...
		const arr_G_NO_NO = ['G2.1', 'G2.2', 'G2.3', 'G2.4', 'G2.6', 'G2.7', 'G2.8', 'G2.9', 'G2.10']; // 1 - нет . 2 - нет
		const arr_G_0_NO_YES = ['G2.1', 'G2.3', 'G2.4', 'G2.6', 'G2.7', 'G2.8', 'G2.9', 'G2.10']; // 1 - нет . 2 = да
		const arr_G_0_NO_YES_2 = ['G2.1', 'G2.3', 'G2.4','G2.5', 'G2.6', 'G2.7', 'G2.8', 'G2.9', 'G2.10'];  // 1 - да . 2 = да
		const arr_G_0_All = ['G2.1','G2.2', 'G2.3', 'G2.4','G2.5', 'G2.6', 'G2.7', 'G2.8', 'G2.9', 'G2.10'];// 1 - да . 2 = нет
			...
			calcWrapE4.forEach(e => {
				...
				if (e.dataset.id == 'E4') {
					...
					if (params.company.industry == 'Кредитные организации' ||
						...
						if (selectInputFunc('E4.1-1').checked) {
							allQuestionsE4.forEach(e => {
	...
	const nextFunc = (elem) => {
		...
		if (filled == true) {
			...
			const wrapsNav = stepWrap.nextElementSibling.querySelectorAll('.wrap-nav');

			if (wrapsNav.length > 0) {
				wrapsNav[0].style.display = 'block';
			}
			var elementClick = '#top';
			var destination = $(elementClick).offset().top;
			$("html:not(:animated),body:not(:animated)").animate({scrollTop: destination}, 300);
	...
	const calculateESG = () => {
		...
		block_E1_bar_line.style.background = E_block_colors[0];
		block_E1_bar_line.style.borderLeftColor = E_block_colors[0];
		...
		block_E2_bar_line.style.background = E_block_colors[1];
		block_E2_bar_line.style.borderLeftColor = E_block_colors[1];
		...
		block_E3_bar_line.style.background = E_block_colors[2];
		block_E3_bar_line.style.borderLeftColor = E_block_colors[2];
		...
		block_E4_bar_line.style.background = E_block_colors[3];
		block_E4_bar_line.style.borderLeftColor = E_block_colors[3];
		...
		block_E5_bar_line.style.background = E_block_colors[4];
		block_E5_bar_line.style.borderLeftColor = E_block_colors[4];
		...
		if (params.block_S['S4']) {
			...
			block_S4_bar_line.style.background = S_block_colors[3];
			block_S4_bar_line.style.borderLeftColor = S_block_colors[3];

			const block_S4_bar_line_text = document.querySelector('.bar_S4 p');
			block_S4_bar_line_text.textContent = Math.round(block_S4_bar) + '%';
		} else {
			block_S4_bar_line.closest('.result-wrap-item-bar').remove();
		...
		block_S1_bar_line.style.background = S_block_colors[0];
		block_S1_bar_line.style.borderLeftColor = S_block_colors[0];
		...
		block_S2_bar_line.style.background = S_block_colors[1];
		block_S2_bar_line.style.borderLeftColor = S_block_colors[1];
		...
		block_S3_bar_line.style.background = S_block_colors[2];
		block_S3_bar_line.style.borderLeftColor = S_block_colors[2];
		...
		block_S5_bar_line.style.background = S_block_colors[4];
		block_S5_bar_line.style.borderLeftColor = S_block_colors[4];
		...
		block_S6_bar_line.style.background = S_block_colors[5];
		block_S6_bar_line.style.borderLeftColor = S_block_colors[5];
		...
		if (params.block_G['G8']) {
			...
			block_G8_bar_line.style.background = G_block_colors[7];
			block_G8_bar_line.style.borderLeftColor = G_block_colors[7];
		...
		const block_G1_bar_line = document.querySelector('.bar_G1');
		block_G1_bar_line.style.width = block_G1_bar + '%';
		block_G1_bar_line.style.background = G_block_colors[0];
		block_G1_bar_line.style.borderLeftColor = G_block_colors[0];

		const block_G1_bar_line_text = document.querySelector('.bar_G1 p');
		...
		block_G2_bar_line.style.background = G_block_colors[1];
		block_G2_bar_line.style.borderLeftColor = G_block_colors[1];
		...
		block_G3_bar_line.style.background = G_block_colors[2];
		block_G3_bar_line.style.borderLeftColor = G_block_colors[2];

		const block_G3_bar_line_text = document.querySelector('.bar_G3 p');
		...
		block_G4_bar_line.style.background = G_block_colors[3];
		block_G4_bar_line.style.borderLeftColor = G_block_colors[3];
		...
		block_G5_bar_line.style.background = G_block_colors[4];
		block_G5_bar_line.style.borderLeftColor = G_block_colors[4];
		...
		block_G6_bar_line.style.background = G_block_colors[5];
		block_G6_bar_line.style.borderLeftColor = G_block_colors[5];
		...
		block_G7_bar_line.style.background = G_block_colors[6];
		block_G7_bar_line.style.borderLeftColor = G_block_colors[6];
	...
	const ratingFunc = (value) => {
		const lowColor = 'C - Начальный',
					lowMidColor = 'B - Развивающийся',
					midColor = 'B - Развивающийся',
					regColor = 'B - Развивающийся',
					hightColor = 'А - Продвинутый',
					verHightColor = 'А - Продвинутый',
					maxColor = 'А - Продвинутый';
	...
	const miniBarFunc = () => {
		...
		for (item in questions) {
			...
			if (bar) {
				for (answer in questions[item].answers) {
					...
					newBar.innerHTML = `
						<div class="left">
							<div class="bar">
								<div class="inner" style="background: ${E_block_colors[index]};width: ${+questions[item].answers[answer] * 100}%">
									<p>${+questions[item].answers[answer] * 100}%</p>
								</div>
							</div>
						</div>
						<div class="right">
			...
			/* ${colorFunc(+questions[item].answers[answer] * 100)} */
		}
		let reg = true;
		for (item in questionsS) {
			const bar = document.querySelector(`#bar_${item}`);
			const bar4 = document.querySelector(`#bar_S4`);
			if (bar) {
				for (answer in questionsS[item].answers) {
					...
					newBar.innerHTML = `
						<div class="left">
							<div class="bar">
								<div class="inner" style="border-left-color: ${S_block_colors[indexS]}; background: ${S_block_colors[indexS]}; width: ${+questionsS[item].answers[answer] * 100}%">
									<p>${+questionsS[item].answers[answer] * 100}%</p>
								</div>
							</div>
						</div>
						<div class="right">
			...
			if (bar) {
				for (answer in questionsG[item].answers) {
					...
					newBar.innerHTML = `
						<div class="left">
							<div class="bar">
								<div class="inner" style="background: ${G_block_colors[indexG]};width: ${+questionsG[item].answers[answer] * 100}%">
									<p>${+questionsG[item].answers[answer] * 100}%</p>
								</div>
							</div>
						</div>
						<div class="right">
	...
	const diagrammFunc = (E_calc, S_calc, G_calc, ESG_calc) => {
		...
		const diagEText = document.getElementById('textE');
		const diagSText = document.getElementById('textS');
		const diagGText = document.getElementById('textG');
		const diagESGText = document.getElementById('textESG');
		
		diagEText.textContent = ratingFunc((E_calc * 100));
		diagSText.textContent = ratingFunc((E_calc * 100));
		diagGText.textContent = ratingFunc((E_calc * 100));
		diagESGText.textContent = ratingFunc((E_calc * 100));
		...
		for (let key in params.block_E) {
			...
			chartList_E.forEach((e,index) => {
				if (e.dataset.id == key) {
					e.style.display = 'flex';
					const color = e.querySelector('.color');
					color.style.background = E_block_colors[index];
				}
			})
		...
		for (let key in params.block_S) {
			...
			chartList_S.forEach((e,index) => {
				if (e.dataset.id == key) {
					e.style.display = 'flex';
					const color = e.querySelector('.color');
					color.style.background = S_block_colors[index];
				}
			})
		}
		...
		for (let key in params.block_G) {
			...
			chartList_G.forEach((e,index) => {
				if (e.dataset.id == key) {
					e.style.display = 'flex';
					const color = e.querySelector('.color');
					color.style.background = G_block_colors[index];
				}
			})
		}
		...
		const data_ESG = {
			...
			data: {
					...
					datasets: [{
							...
							backgroundColor: ['#769240', '#DD7230', '#485053'],
							offset: 0,
							borderWidth: 2,
...
Вы можете с этого браузера переходить по данной ссылке в любое время, что бы снова посмотреть или скачать отчет. Ваша ссылка: http://${domain}/result`);
			printJS({
				printable: 'calcs',
				type: 'html',
